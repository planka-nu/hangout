	...
	socket.on('connect', function() {
		...
		socket.on('join', function (room) {
			...
			if (typeof roomDivs[room] === 'undefined') {
				...
				roomDivs[room] = $('<div id="'+room+'" class="my2 brdr--light-gray bg--white"><div class="header bg--off-white py1"><div class="grd"><div class="grd-row"><div class="grd-row-col-4-6"><h1></h1></div><div class="grd-row-col-2-6 txt--right py1-5"><a href="#" class="leaveroom btn btn--gray">Leave room</a></div></div></div></div><div class="attendance-list m1">Attendance: <ul class="attendance"></ul></div><div class="m1"><ul class="msgs"></ul></div><div class="footer bg--off-white py1"><div class="grd"><div class="grd-row"><div class="grd-row-col-4-6"><input type="text" class="msg" placeholder="Say something..." /></div><div class="grd-row-col-2-6"><div class="right"><button class="btn btn--green attendance-button r-Tooltip r-Tooltip--top" data-r-tooltip="Lista alla som deltar"><i>0</i></button></div><div class="left"><input type="button" class="btn btn--blue msg-send" value="Say!" /></div></div><span class="writingSummary"></span></div></div></div>');
		...
		socket.on('attendance', function (room, list) {
			...
			$('.attendance-button i', roomDivs[room]).html( list.length );
			$('.attendance-button', roomDivs[room]).attr('data-r-tooltip' ,list.toString().replace(/,/g, ' '));
			/*
